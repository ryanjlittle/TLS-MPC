...
void run_aes(int party, int port, string key_share, string plaintext) {
    ...
    // plaintext
	for(int i = 0; i < 128; i++) {
		in.assign_plaintext_bit(i, plaintext[i]=='1');
	}
    // key
	for(int i = 0; i < 128; i++) {
...
int main(int argc, char* argv[]) {
    ...
    std::string plaintext = hex_to_binary(string(argv[4]));
	...
	run_aes(party, port, key_share, plaintext);
